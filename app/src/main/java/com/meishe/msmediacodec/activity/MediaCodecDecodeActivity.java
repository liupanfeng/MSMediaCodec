...
import com.meishe.msmediacodec.AudioTrackPlayer;
...
public class MediaCodecDecodeActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    private String mFilePath="/sdcard/DCIM/14.mp4";
    private MediaExtractor mMediaExtractor;
    private MediaExtractor mAudioMediaExtractor;
    private MediaCodec mMediaCodec;
    private MediaCodec mAudioCodec;
    private AudioTrackPlayer mAudioTrackPlayer;
    private DecodeVideoThread mDecodeThread;
    private DecodeAudioThread mDecodeAudioThread;
...
//        setContentView(R.layout.activity_media_codec_decode);
        ...
        mAudioTrackPlayer.initAudioTrack();
    }
...
//            mDecodeThread =new DecodeVideoThread(holder.getSurface());
//            mDecodeThread.start();
//        }
        if (mDecodeAudioThread ==null){
            mDecodeAudioThread =new DecodeAudioThread();
            mDecodeAudioThread.start();
        }
...
//        startSyncPlay(holder.getSurface());
    ...
    public void surfaceDestroyed(@NonNull SurfaceHolder holder) {
        ...
        if (mDecodeAudioThread !=null){
            mDecodeAudioThread.interrupt();   //停止线程的正确姿势
        }
    }
...
//                    }
//                    //这样配置之后，解码之后的数据就会 直接显示在mSurface 上边  这里是核心点
//                    mAudioCodec.configure(trackFormat, null,null,0);
//                    mAudioCodec.setCallback(new MediaCodec.Callback() {
//                        @Override
//                        public void onInputBufferAvailable(@NonNull MediaCodec codec, int index) {
//                            Log.d("lpf","audio onInputBufferAvailable");
//                            ByteBuffer inputBuffer = codec.getInputBuffer(index);
//                            int sampleSize=mMediaExtractor.readSampleData(inputBuffer,0);  //从MediaExtractor中从数据读出来，读取到inputBuffer中
//                            if (sampleSize>0) {
//                                codec.queueInputBuffer(index,0,sampleSize,mMediaExtractor.getSampleTime(),0); //进行解码操作
//                                mMediaExtractor.advance();  //下一帧数据
//                            }else {
//                                codec.queueInputBuffer(index,0,0,0,MediaCodec.BUFFER_FLAG_END_OF_STREAM);
//                            }
...
//                        @Override
//                        public void onOutputBufferAvailable(@NonNull MediaCodec codec, int index, @NonNull MediaCodec.BufferInfo info) {
//                            Log.d("lpf","audio onOutputBufferAvailable");
//                            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0){
//                                Log.d("lpf","audio outputBuffer BUFFER_FLAG_END_OF_STREAM");
//                                codec.stop();
//                                codec.release();// 释放组件
//                                mMediaExtractor.release();
...
//                            }
//
//                            ByteBuffer outputBuffer = codec.getOutputBuffer(index);   //这个就是解码之后的数据
//                            Log.d("lpf","outputBuffer.array().length---"+outputBuffer.array().length);
//                            mAudioTrackPlayer.write(outputBuffer.array(), info.size, AudioTrack.WRITE_BLOCKING);
...
//                } catch (IOException e) {
//                    e.printStackTrace();
//                    Log.e("lpf","audio play error:"+e.getMessage());
//                }
    ...
    private class DecodeVideoThread extends Thread{

        private Surface mSurface;

        /*通过构造方法将surface传递进来*/
        public DecodeVideoThread(Surface surface){
            mSurface = surface;
        }
...
//                    如果数据的buffer可用，将无限期等待timeUs的单位是纳秒
    ...
    private class DecodeAudioThread extends Thread{
        ...
        public void run() {
            ...
            try {
                mAudioMediaExtractor.setDataSource(mFilePath);
            } catch (IOException e) {
                e.printStackTrace();
            }
            int trackCount = mAudioMediaExtractor.getTrackCount();
            //从媒体提取器中拿到了 MIME 以及MediaFormat   通过MIME 创建的硬件解码器   通过MediaFormat配置的硬件解码器
            for (int i = 0; i < trackCount; i++) {
            ...
            while (!Thread.interrupted()){//只要线程不中断
                ...
                if (!isEOF){
                    ...
                    //传入的timeoutUs为0表示立即返回
//                    如果数据的buffer可用，将无限期等待timeUs的单位是纳秒
                    int index =mAudioCodec.dequeueInputBuffer(10000);
                    if (index >= 0){
                        ByteBuffer byteBuffer=inputBuffers[index];
                        Log.d("lpf","bytebuffer is "+byteBuffer);
                        int sampleSize=mAudioMediaExtractor.readSampleData(byteBuffer,0);
                        Log.d("lpf","sampleSize is "+sampleSize);
                        if (sampleSize < 0){
                            Log.d("lpf","inputBuffer is BUFFER_FLAG_END_OF_STREAMING");
                            mAudioCodec.queueInputBuffer(index,0,0,0,MediaCodec.BUFFER_FLAG_END_OF_STREAM);
                            isEOF=true;
                        }else{
                            mAudioCodec.queueInputBuffer(index,0,sampleSize,mAudioMediaExtractor.getSampleTime(),0);
                            mAudioMediaExtractor.advance();  //下一帧数据
                        }
                    }
                }

                int outIndex=mAudioCodec.dequeueOutputBuffer(info,100000);
                switch (outIndex){
                    case MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED:
                        //当buffer变化时，必须重新指向新的buffer
                    ...
                    default:
                        ...
                        //由于配置的时候 将Surface 传进去了  所以解码的时候 将数据直接交给了Surface进行显示了
                        //使用简单的时钟的方式保持视频的fps（每秒显示的帧数），不然视频会播放的比较快
                        Log.d("lpf","音频解码之后的 buffer数据="+buffer);
//                        while (info.presentationTimeUs/1000>System.currentTimeMillis()-startMs){
//                            try {
//                                Thread.sleep(10);
//                            } catch (InterruptedException e) {
...
//                            }
//                        }
                        mAudioTrackPlayer.write(buffer.array(),info.size, AudioTrack.WRITE_BLOCKING);
